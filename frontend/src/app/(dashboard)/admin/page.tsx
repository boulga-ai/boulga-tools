  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 py-6 md:px-8 md:py-8">
      <div>
        ...
        <p className="text-muted-foreground">Indicateurs clés de la plateforme.</p>
      </div>
      ...
      {!kpis ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            ...
            <StatTile label="Coût du jour" value={formatUsd(kpis.costs.today_usd)} />
            <StatTile label="Coût du mois" value={formatUsd(kpis.costs.month_usd)} />
          </div>

          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="rounded-[12px] border bg-card p-5">
              <h3 className="mb-4">Coût par outil (30 derniers jours)</h3>
              <BarChart
            ...
            <div className="rounded-[12px] border bg-card p-5">
              <h3 className="mb-4">Claude vs autres modèles</h3>
              <BarChart
                data={[
                  { label: "Claude", value: kpis.costs.claude_usd },
                  { label: "Autres modèles", value: kpis.costs.other_models_usd },
                ]}
                formatValue={formatUsd}
              />
            </div>
          ...
          <div className="rounded-[12px] border bg-card p-5">
            <h3 className="mb-4">Répartition des utilisateurs par palier</h3>
            <ProportionBar byTier={kpis.users.by_tier} />
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <StatTile label="Générations (30j)" value={kpis.volumes.generations_30d.toLocaleString("fr-FR")} />
            <StatTile
              label="Mots générés (30j)"
              value={kpis.volumes.words_generated_30d.toLocaleString("fr-FR")}
            />
            <StatTile
              label="Documents téléchargés (30j)"
